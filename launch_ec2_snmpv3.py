...
def lambda_handler(event, context):
    ...
    auth_algorithm = event['auth-algorithm']
    auth_password = event['auth-password']
    priv_algorithm = event['priv-algorithm']
    priv_password = event['priv-password']
    hostname = event['hostname']

    # Set up the user data script to configure SNMPv3
    user_data_script = f"""#!/bin/bash
        yum update -y
        yum install -y net-snmp net-snmp-utils
        mv /etc/snmp/snmpd.conf /etc/snmp/snmpd.conf.original
        sudo sh -c "echo 'createUser {username} {auth_algorithm} {auth_password} {priv_algorithm} {priv_password}' >> /etc/snmp/snmpd.conf"
